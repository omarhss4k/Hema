...
box(s, "Pipeline : Souris  →  Rat  →  Humain  |  Modèle de Friberg et al. + Simeoni",
...
cols = [
    ...
    (Inches(4.7),  "Transfert Rat → Humain",   VERT,
    ...
    (Inches(9.0),  "Étape 4.2 — Simeoni",      VERT,
     ["3 bugs identifiés & corrigés",
      "k₁ = 0.638  k₂ = 0.00106",
      "Proche valeurs publiées (k₁≈0.7)",
      "Script R prêt pour l'ADC"]),
]

for cx, titre, col, items in cols:
    rect(s, cx, Inches(1.5), Inches(4.0), Inches(0.5), col)
    box(s, titre, cx + Inches(0.1), Inches(1.52), Inches(3.8), Inches(0.45),
...
# ═════════════════════════════════════════════════════════════════════════════
# SLIDE 3 — ÉTAPE 4.2 : VALIDATION SIMEONI
# ═════════════════════════════════════════════════════════════════════════════
s = prs.slides.add_slide(BLANK)
bg(s, BLANC)
header_bar(s, "Étape 4.2 — Validation du modèle de Simeoni",
           "Irinotécan (données publiques) → validation avant application à l'ADC")
footer(s)

# 3 problèmes
problems = [
    ("01", "w₀ incorrecte",          "Fixée à 0.081 g → vraies données à 0.114 g\nDécalage systématique du modèle"),
    ("02", "Point mal ordonné PD60", "Solveur ODE plantait silencieusement\nCourbes parasites sans message d'erreur"),
    ("03", "Optimiseur local",       "nlminb coincé dans faux minimum\n→ doses 45 et 60 mg/kg indiscernables"),
]
fixes = [
    ("01", "Extraction de w₀",    "Régression sur premières mesures\n→ w₀ = 0.114 g depuis les données"),
    ("02", "Tri systématique",    "order(time) à chaque lecture\n→ ODE toujours stable"),
    ("03", "DEoptim (global)",    "Algorithme évolutionnaire\n→ différenciation des 2 doses"),
]

# Colonnes problème / correction
box(s, "Problèmes identifiés", Inches(0.4), Inches(1.45), Inches(5.8), Inches(0.4),
    font_size=14, bold=True, fg=ORANGE)
box(s, "Corrections apportées", Inches(6.8), Inches(1.45), Inches(5.8), Inches(0.4),
    font_size=14, bold=True, fg=VERT)

# Séparateur vertical
rect(s, Inches(6.45), Inches(1.4), Inches(0.04), Inches(4.5), BLEU_CLAIR)

for i, ((n, t, d), (_, tf, df)) in enumerate(zip(problems, fixes)):
    y = Inches(1.9) + Inches(1.35) * i
    # problème
    rect(s, Inches(0.4), y, Inches(0.55), Inches(0.55), ORANGE)
    box(s, n, Inches(0.4), y, Inches(0.55), Inches(0.55),
        font_size=13, bold=True, fg=BLANC, align=PP_ALIGN.CENTER)
    box(s, t, Inches(1.05), y, Inches(5.1), Inches(0.32),
        font_size=12, bold=True, fg=BLEU)
    box(s, d, Inches(1.05), y + Inches(0.3), Inches(5.1), Inches(0.6),
        font_size=11, fg=RGBColor(0x44, 0x44, 0x44))
    # correction
    rect(s, Inches(6.8), y, Inches(0.55), Inches(0.55), VERT)
    box(s, "✔", Inches(6.8), y, Inches(0.55), Inches(0.55),
        font_size=13, bold=True, fg=BLANC, align=PP_ALIGN.CENTER)
    box(s, tf, Inches(7.45), y, Inches(5.1), Inches(0.32),
        font_size=12, bold=True, fg=BLEU)
    box(s, df, Inches(7.45), y + Inches(0.3), Inches(5.1), Inches(0.6),
        font_size=11, fg=RGBColor(0x44, 0x44, 0x44))

# Résultat
rect(s, Inches(0.4), Inches(5.95), Inches(12.5), Inches(0.75), BLEU)
box(s, "Résultat :  k₁ = 0.638  (réf. ≈ 0.7)   |   k₂ = 0.00106  (réf. ≈ 0.0015)   →   Estimations biologiquement cohérentes ✔",
    Inches(0.7), Inches(6.05), Inches(12.0), Inches(0.55),
    font_size=13, bold=True, fg=BLANC, align=PP_ALIGN.CENTER)


# ═════════════════════════════════════════════════════════════════════════════
# SLIDE 4 — MODÈLE FRIBERG : RAT
# ═════════════════════════════════════════════════════════════════════════════
s = prs.slides.add_slide(BLANK)
bg(s, GRIS)
...
s = prs.slides.add_slide(BLANK)
bg(s, GRIS)
header_bar(s, "VPC & Analyse de sensibilité — Modèle Humain",
           "Population de 1000 patients | AUC = 4 / 5 / 6")
footer(s)
...
for i, it in enumerate(vpc_items):
    box(s, "▸  " + it, Inches(0.6), Inches(2.05) + Inches(0.45) * i,
        Inches(12.0), Inches(0.4), font_size=12, fg=BLEU)

rect(s, Inches(0.4), Inches(3.3), Inches(12.5), Inches(0.04), BLEU_CLAIR)

box(s, "Analyse de sensibilité sur l'AUC cible",
    Inches(0.4), Inches(3.4), Inches(12.0), Inches(0.4),
    font_size=14, bold=True, fg=BLEU)

aucs = [
    ("AUC = 4", "Faible exposition — toxicité modérée", VERT),
    ("AUC = 5", "Dose standard Calvert (référence)", BLEU_CLAIR),
    ("AUC = 6", "Forte exposition — toxicité accrue", ORANGE),
]
for i, (auc, desc, col) in enumerate(aucs):
    cx = Inches(0.4) + Inches(4.1) * i
    rect(s, cx, Inches(3.9), Inches(3.9), Inches(0.5), col)
    box(s, auc, cx + Inches(0.1), Inches(3.93), Inches(3.7), Inches(0.42),
        font_size=14, bold=True, fg=BLANC)
    box(s, desc, cx + Inches(0.1), Inches(4.5), Inches(3.7), Inches(0.5),
        font_size=11, fg=BLEU)

box(s, "→  Grades NCI-CTCAE calculés pour chaque scénario (Figure 4c)",
    Inches(0.6), Inches(5.5), Inches(12.0), Inches(0.45),
    font_size=12, italic=True, fg=RGBColor(0x44, 0x44, 0x44))
...
recap = [
    ("✔ Simeoni validé", Inches(1.7)),
    ("✔ Rat modélisé",   Inches(4.6)),
    ("✔ Humain transféré", Inches(7.4)),
    ("→ ADC en cours",  Inches(10.4)),
]
for txt, x in recap:
...
rect(s, 0, Inches(6.9), W, Inches(0.6), BLEU_CLAIR)
box(s, "Mars 2026  |  Pipeline : Souris → Rat → Humain  |  Modèle Friberg + Simeoni",
    Inches(0.5), Inches(6.93), Inches(12.3), Inches(0.45),
    font_size=12, fg=BLANC, align=PP_ALIGN.CENTER)
