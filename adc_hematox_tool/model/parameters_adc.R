############################################################
# parameters_adc_human.R
# Paramètres PK/PD -- ADC (ADC de reference) -- HUMAIN
#
...
source("parameters_baseline.R")
source("parameters_fornari.R")

# -- Paramètres PK ADC humain ---------------------------
...
adc_pars_hu <- list(
  ...
  # -- IC50 ADC (ADC entier) -- assay PFB-10 sur HPC ------
...
adc_pars_hu$mgL_to_uM_DXd <- 1000 / adc_pars_hu$MW_DXd
...
p_sensitive_adc     <- 0.20   # G3-4 = 20% × 80% ≈ 16% (pooled FDA 16.2%)
p_moderate_adc      <- 0.14   # G1+G2 = 7%+7% = 14% (FDA pooled)
# p_resistant = 1 - p_sensitive - p_moderate = 0.66

Slope_resist_adc    <- 0.10   # kill_max ≈ 7%  → G0 garanti
Slope_moderate_adc  <- 12.0   # kill intermédiaire → G1-G2
Slope_sensitive_adc <- 59.1   # kill fort → G3-4 (nécessaire pour séparation binaire)

# Compatibilité backward
Slope_CMP_adc_human <- Slope_sensitive_adc
...
p_MEP_light_adc         <- 0.33   # G1 cible (iter.8)
p_sensitive_mep_adc     <- 0.33   # G2-G3 cible (inchangé)
# p_MEP_resistant = 1 - 0.33 - 0.33 = 0.34 → G0 cible (iter.8)

Slope_MEP_resist_adc    <- 0.01   # résistants MEP : quasi-nul → G0 (<0.5% chute RBC)
Slope_MEP_light_adc     <- 0.18   # légers → centre G1 (ω élargi, Slope légèrement baissé)
Slope_MEP_sensitive_adc <- 1.00   # sensibles → G2-G3 (calibré anémie G3-4 ~7.3% FDA pooled N=234)
...
# -- Résumé -----------------------------------------------
cat("╔══════════════════════════════════════════════════════════╗\n")
cat("║  PK/PD ADC -- HUMAIN (Yin 2020 + Vasalou 2024)       ║\n")
cat("╚══════════════════════════════════════════════════════════╝\n\n")
cat("-- ADC (2-compartiments) --\n")
cat(sprintf("  CL_ADC = %.5f L/h  V1 = %.2f L  V2 = %.2f L\n",
            adc_pars_hu$CL_ADC, adc_pars_hu$V1_ADC, adc_pars_hu$V2_ADC))
cat("-- DXd payload --\n")
cat(sprintf("  CL_DXd = %.1f L/h  V_DXd = %.2f L  T½_int = %.2f h\n",
            adc_pars_hu$CL_DXd, adc_pars_hu$V_DXd,
            log(2) * adc_pars_hu$V_DXd / adc_pars_hu$CL_DXd))
cat("-- Mécanistiques --\n")
cat(sprintf("  k_int = %.5f h-  Krel_C1 = %.4f h-\n",
            adc_pars_hu$k_int, adc_pars_hu$k_rel_c1))
cat(sprintf("  V_ic  = %.3f L  (ratio V_DXd/V_ic = %.0f)\n",
            adc_pars_hu$V_ic,
            adc_pars_hu$V_DXd / adc_pars_hu$V_ic))
...
cat(sprintf("  IC50_ADC : ery=%.1f µg/mL  neut=%.1f µg/mL  shared=%.2f µg/mL\n",
            adc_pars_hu$IC50_ADC_ery_ugmL,
            adc_pars_hu$IC50_ADC_neut_ugmL,
            adc_pars_hu$IC50_ADC_ugmL))
cat(sprintf("  C_Avg_clinique = 33.3 µg/mL → E_drug_avg = %.3f\n\n",
            33.3 / (adc_pars_hu$IC50_ADC_ugmL + 33.3)))
