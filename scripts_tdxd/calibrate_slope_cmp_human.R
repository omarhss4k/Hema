...
cat(sprintf("  %-10s  %-10s  %-12s  %-12s  %s\n",
            "Slope_CMP", "G3-4 %", "Tout grade %", "Neut médiane", "Statut"))
cat(sprintf("  %-10s  %-10s  %-12s  %-12s  %s\n",
            "─────────", "──────────","────────────","────────────","──────"))
...
for (s in slope_vals) {
  ...
  cat(sprintf("\r  %-10.0f  %-10.1f  %-12.1f  %-12.2f  %s\n",
              s, r$g34, r$g_any, r$med_nadir, status))
}
...
# Trouver les deux valeurs encadrant 20%
...
if (length(below) > 0 && length(above) > 0) {
  ...
  cat(sprintf("\n  Interpolation : Slope_CMP_tdxd_human ≈ %.1f\n", slope_opt))
  cat(sprintf("  (entre %.0f [G3-4=%.1f%%] et %.0f [G3-4=%.1f%%])\n",
              s_low, g_low, s_high, g_high))
} else if (length(below) == 0) {
  cat("\n  ATTENTION : même Slope=5 donne G3-4 > 20%\n")
  slope_opt <- slope_vals[which.min(abs(g34_vec - 20))]
} else {
  cat("\n  ATTENTION : G3-4 < 20% pour toutes les valeurs testées — augmenter la plage\n")
  slope_opt <- max(slope_vals)
}

cat(sprintf("\n  → Slope_CMP_tdxd_human = %.1f\n", slope_opt))
cat(sprintf("    (carboplatin humain = 0.57 — T-DXd plus sensible sur la CMP)\n"))
cat("═══════════════════════════════════════════════════════════\n")
