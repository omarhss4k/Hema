...
for (i in 1:N) {
  ...
  # Mixture kmax_CMP (neutropenie) : proportions FDA (66/14/20)  [iter.3]
  rc<-runif(1)
  pt$kmax_CMP <- if(rc<0.66) 0.02*exp(rnorm(1,0,0.3))
                 else if(rc<0.80) 0.18*exp(rnorm(1,0,0.3))
                 else 0.68*exp(rnorm(1,0,0.35))
  # Mixture kmax_ret_prol (anemie) : proportions FDA (34/33/33)  [iter.3]
  rm<-runif(1)
  pt$kmax_ret_prol <- if(rm<0.34) 0.005*exp(rnorm(1,0,0.3))
                      else if(rm<0.67) 0.035*exp(rnorm(1,0,0.3))
                      else 0.20*exp(rnorm(1,0,0.5))
