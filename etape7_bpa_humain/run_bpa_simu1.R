...
times_j  <- times_hu / 24   # en jours pour les figures
...
# Matrices profils : une colonne par patient
...
for (i in 1:N_patients) {
  ...
  if (!is.null(out) && nrow(out) > 10) {
    ...
    # Stocker profil complet si lsoda a convergé jusqu'au bout
    if (nrow(out) >= n_times) {
...
n_ok    <- nrow(results)
# Supprimer colonnes sans profil
...
cat("\n=============================================================\n")
cat(sprintf("  BPA SIMU 1 (sous-traitant) -- %.1f mg/kg Q3W x %d (N=%d)\n",
...
cat(sprintf("    Neut nadir : %.2f [%.2f-%.2f] x 10^9/L\n", q_neut[2], q_neut[1], q_neut[3]))
cat(sprintf("    RBC/RBC0   : %.2f [%.2f-%.2f]\n", q_rbc[2], q_rbc[1], q_rbc[3]))
cat("=============================================================\n")

# ══════════════════════════════════════════════════════════
# Helper : bande VPC (P10-P50-P90) sur une matrice profil
# ══════════════════════════════════════════════════════════
.vpc_band <- function(mat, times_j, col_med, col_band,
  ...
  # Marques cycles (J0, J21, J42, J63, J84, J105)
  cycle_days <- (0:(N_CYCLES - 1)) * (INTERVAL_H / 24)
  abline(v = cycle_days, lty = 3, col = "grey70")

  legend("topright",
         legend = c("Mediane", "P10-P90"),
         col    = c(col_med, col_band),
         lwd    = c(2, 8), bty = "n", cex = 0.8)
}
...
pdf("results/BPA_simu1_VPC.pdf", width = 14, height = 10)
...
.vpc_band(mat_adc, times_j,
          ...
          ylab = "ADC [ug/mL]",
          main = sprintf("BPA Simu1 : PK ADC (%.1f mg/kg Q3W x%d)", DOSE_MGKG, N_CYCLES))

# 2. Neutrophiles
.vpc_band(mat_neut, times_j,
          col_med  = "#2166ac",
          col_band = adjustcolor("#2166ac", alpha.f = 0.20),
          ylab = "Neutrophiles [x 10^9/L]",
          main = "BPA Simu1 : Neutrophiles",
...
.vpc_band(mat_rbc, times_j,
          ...
          ylab = "RBC [x 10^9/L]",
          main = "BPA Simu1 : Erythrocytes (proxy anemie)",
...
# ── Page 2 : grades + nadirs ─────────────────────────────
par(mfrow = c(2, 3), mar = c(4, 4.2, 3, 1))

# 5. Distribution nadir Neut
...
# 10. Barplot grades Thrombocytopenie
bp <- barplot(pct_p[grade_order], col = grade_cols[grade_order],
              names.arg = grade_order, ylim = c(0, 100),
              main = "Grades Thrombocytopenie BPA Simu1", ylab = "Patients (%)")
