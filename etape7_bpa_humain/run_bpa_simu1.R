...
# PD : Fornari 2019 + Slopes calibres par ratio IC50 vs T-DXd
...
# -- Matrices profils (8 compartiments) ----------------------
...
for (i in 1:N_patients) {
  ...
  pars_i$Slope_CMP <- max(0, Slope_CMP_BPA_s1 * exp(rnorm(1, 0, omega_Slope_CMP)))
  pars_i$Slope_MEP <- max(0, Slope_MEP_BPA_s1 * exp(rnorm(1, 0, omega_Slope_MEP)))
...
# ══════════════════════════════════════════════════════════
# Helper figure : profil log-scale style Fornari
# ══════════════════════════════════════════════════════════
...
pdf("results/BPA_simu1_VPC.pdf", width = 16, height = 9)
...
if (has_profiles) {
  ...
  mtext(sprintf("BPA Simu1 -- %.1f mg/kg Q3W x%d  |  Slope_CMP=%.0f  Slope_MEP=%.3f  (N=%d)",
                DOSE_MGKG, N_CYCLES, Slope_CMP_BPA_s1, Slope_MEP_BPA_s1, n_ok),
...
hist(results$Neut_nadir, breaks=30, col="#74add1", border="white",
     main="Nadir Neutrophiles", xlab="Neut nadir [x 10^9/L]", ylab="Frequence")
...
hist(pct_drop, breaks=30, col="#f4a582", border="white",
     main="Chute RBC (%Delta baseline)", xlab="%Delta RBC", ylab="Frequence")
