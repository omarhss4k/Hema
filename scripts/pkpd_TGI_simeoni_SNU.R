...
#   dx2/dt =  k2·C · x1 - k1·x2             ← transit 1
...
# Repérer les deux lignes d'en-tête (contiennent "Time")
...
tv_ctrl   <- num_col(b1, 2)   # Vehicule
tv_d1p2   <- num_col(b1, 3)   # Group 4 = 1.2 mg/kg
tv_d5p6   <- num_col(b1, 4)   # Group 5 = 5.6 mg/kg
tv_d11p8  <- num_col(b1, 6)   # Group 7 = 11.8 mg/kg  (col 5 vide)
...
# =============================================================================
# 3. ÉQUATIONS DU MODÈLE DE SIMEONI (deSolve)
# =============================================================================
...
k1_init <- 4 / 7
Cmax_11p8 <- 11800 / as.numeric(pk_fixed["V1"])
k2_init   <- L0_init / Cmax_11p8

init_pd <- c(L0 = L0_init, L1 = L1_init, k1 = k1_init, k2 = k2_init)

cat("\n=== Valeurs initiales PD (Simeoni) ===\n")
...
t1_test <- sim_treated(1200, tv0_d1p2, params_test, times_d[ok_d1p2])
cat(sprintf("1.2 mg/kg : %s\n",
    if (any(is.na(t1_test))) "ECHEC (NA)" else
    paste(round(head(t1_test, 4)), collapse = " | ")))

t5_test <- sim_treated(5600, tv0_d5p6, params_test, times_d[ok_d5p6])
cat(sprintf("5.6 mg/kg : %s\n",
    if (any(is.na(t5_test))) "ECHEC (NA)" else
    paste(round(head(t5_test, 4)), collapse = " | ")))

t11_test <- sim_treated(11800, tv0_d11p8, params_test, times_d[ok_d11p8])
cat(sprintf("11.8mg/kg : %s\n",
    if (any(is.na(t11_test))) "ECHEC (NA)" else
    paste(round(head(t11_test, 4)), collapse = " | ")))

obj0 <- objective_simeoni(log(init_pd))
cat(sprintf("Objectif initial : %.4f %s\n", obj0,
    if (obj0 >= 1e11) "— ECHEC (simulations NA)" else "— OK"))
...
cat("\n=== Paramètres PD estimés (Simeoni) — SNU ===\n")
