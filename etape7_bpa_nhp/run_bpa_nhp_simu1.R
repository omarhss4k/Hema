...
cat(sprintf("\n[NHP Simu 1] IC50_CMP=%.1f nM  Slope_CMP=%.0f\n",
            IC50_CMP_BPA_s1, Slope_CMP_BPA_s1))
...
saveRDS(results, "results/bpa_nhp_simu1_results.rds")
...
cat("\n=============================================================\n")
cat(sprintf("  BPA NHP SIMU 1 -- %.1f mg/kg %s x%d (N=%d animaux)\n",
...
pdf("results/BPA_NHP_simu1_VPC.pdf", width = 16, height = 9)
...
if (has_profiles) {
  ...
  mtext(sprintf("BPA NHP Simu1 -- %.1f mg/kg %s x%d  |  Slope_CMP=%.0f  Slope_MEP=%.3f  (N=%d)",
                DOSE_MGKG, FREQ_LABEL, N_CYCLES, Slope_CMP_BPA_s1, Slope_MEP_BPA_s1, n_ok),
...
dev.off()
cat("  -> results/BPA_NHP_simu1_VPC.pdf\n")
