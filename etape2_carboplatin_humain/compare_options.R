...
make_individual_figure(res1, "results/Compare_Option1_doseFixe.pdf",
  subtitle = sprintf("Dose fixe = %.0f mg (AUC=5, GFR_ref=125), omega_CL=0.35, omega_Slope=0.55, n=%d",
                     dose_ref, N_PAT))

make_individual_figure(res2, "results/Compare_Option2_GFR125.pdf",
  subtitle = sprintf("GFR~N(125,25), omega_CL=0.35, omega_Slope=0.55, n=%d", N_PAT))

make_individual_figure(res3, "results/Compare_Option3_slopeLarge.pdf",
  subtitle = sprintf("GFR~N(78,20), omega_CL=0.35, omega_Slope=1.20, n=%d", N_PAT))

make_comparison_figure(
  list(res1, res2, res3),
  "results/Compare_3options_vs_Fornari.pdf"
)
